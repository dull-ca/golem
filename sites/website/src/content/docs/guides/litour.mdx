...
`scaly` and `talos` are also the two hosts you can run for real: both sit on
the public `niklasf/fishnet` image, while the `ghcr.io` refs the other four
carry are not publicly pullable. This is an illustrative topology, not a
deployable one.
