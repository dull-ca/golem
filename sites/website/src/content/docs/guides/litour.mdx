...
<Snippet file="litour-helpers.emet" region="helpers" />
...
golem also ships [`Traefik`](/reference/ingress/), which models a TLS front door
and per-hostname routing as typed values. lichess predates it and keeps these
nginx glyphs; both lower to the same four.
...
main : List Scroll
main = [ scaly, manta, orbit, talos, kaiju, remora ]
```
...
## Cross-host facts are ordinary values
...
`service`'s firewall opening reads `internalNetwork` as imported data, checked
by the type system and substituted while the fleet compiles. The CIDR is in the
`.nft` file before the manifest exists.
...
`scaly`'s single workload lowers to three glyphs:

<Output file="litour.text.golden" scroll="scaly" />

`manta` — the same helpers, but two workloads, two services, and two ingresses
— lowers to forty-six. Both blocks are the compiler's own output, cut from one
run over `examples/lichess/fleet.emet`:

<Output file="litour.text.golden" scroll="manta" />
...
- **Helpers are yours.** `dockerImage`, `service`, `ingress` are this fleet's
  own layer-c shortcuts, written in Emet like anything else and reusable
  across its hosts.
- **Ingress stays hand-rolled.** A reverse proxy is another container's
  config, so this fleet writes those glyphs directly; `Quadlet` models
  container runtimes.
