...
1. **Build the site on your workstation.**
   ...
   The devenv script that runs `bun run build` in `sites/website`. Output lands
   in `sites/website/dist`.

2. **Ship the build context to the builder and push the image.**
...
- **Reverse the web box.** Apply a `web` scroll with an empty `glyphs` list and
  watch golem stop the container, remove the quadlet, close `:80`, and remove
  exactly its ingress line from the shared chain — leaving any other host's line
  intact.
