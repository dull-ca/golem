...
3. **Look at the fleet.**
   ...
   `content-id` and `glyphs` because nothing has been applied. Every one of
   those `reachable` cells was answered through an ssh forward the harness
   opened and tore down for that single request.
...
a port and hands ssh nothing else. Against a VM that fails with `Host key
