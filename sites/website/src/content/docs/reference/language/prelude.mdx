...
language and are supplied here (ADR 0006). Names carrying a `List.` / `Maybe.` /
...
(ADR 0005); they are in scope to build and to pattern-match.
...
grapheme offset — a combining mark counts as its own scalar (ADR 0025). No
...
reaches the wire (ADR 0025). The `is*` predicates are Elm's ASCII-oriented
definitions.
