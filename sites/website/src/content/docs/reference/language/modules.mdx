...
A bare `Thing` or a bare `Wrap` in that module is a compile error naming both
candidates and offering both spellings. It is reported where the name is
*written*, not at the `import` — the imports are fine, and only a use site can
be ambiguous. Renaming one of the two remains available and is often the
clearer answer.
