...
through `module` / `import`. See ADR 0016.
...
matched in the importer. See ADR 0017.
...
See ADR 0006 and the [prelude](/reference/language/prelude/).

## The `main` entry point

Exactly one module — the entry — declares `main : List Scroll`. Every other
module is a library. A library that declares `main` is a compile error. See
ADR 0009.
...
`import Foo` resolves `Foo.emet` over a search path, first match winning. See
ADR 0024.
...
directories. Without an `emet.json`, resolution is entry-directory-only — the
original ADR 0016 behavior.
