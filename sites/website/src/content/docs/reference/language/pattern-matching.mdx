...
| Int literal | an equal `Int` | `0` |
| Float literal | an equal `Float` | `3.14` |
...
`Char` and `String` literal *patterns* differ: string literal patterns match,
but `Char` literal patterns are not yet supported (a `Char` scrutinee is
matched with a variable or wildcard). See ADR 0025.
