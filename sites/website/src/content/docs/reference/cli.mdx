...
`--against-host` reads files directly, without the per-path lock an apply
takes on the same file, so a plan racing a live apply can read a file
mid-rewrite and report a spurious `≠ differs` on that one row. Nothing is
written either way, so this is a snapshot inaccuracy, not a safety issue —
it self-corrects on the next plan.
