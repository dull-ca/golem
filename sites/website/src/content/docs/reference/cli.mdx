...
A table carries `url` **or** `ssh`, never both. The name is the join key:
it matches the scroll of the same name in the manifest. See
...
`<addr>` takes two shapes. `http://host:port` dials directly — useful
against a local agent, and nothing else, since a deployed one binds
loopback. `ssh://[user@]host[:port]` makes `golemctl` open its own SSH
forward to the agent's loopback port and speak HTTP through it; the port
there is **ssh's**, not the agent's.
...
Every request carries `Authorization: Bearer <token>`, taken from
`GOLEM_AUTH_TOKEN`, from the file named by `GOLEM_AUTH_TOKEN_FILE`, or from
a per-host inventory `token_file`. A `401` comes back as an error naming
those. See [Trust model](/explanation/trust/).
...
Options:
      ...
      --config <FILE>          golemd.toml: [retry], [enact], [auth]
