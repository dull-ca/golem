...
Compile it and look at the plan:
...
There is no built-in "container," "service," or "ingress." Those are Emet
functions that return a `List Glyph`. A container workload, for example,
lowers to a podman package, a quadlet `file`, and a `systemdService`:
...
Read it, copy it, change it. The [service abstraction guide](/guides/app-and-db/)
builds this up in full.
