...
fn in_place_file_replace_is_one_apply_step_capturing_the_prior_bytes() {
    ...
    f.apply_manifest(&manifest(vec![file("/etc/app.conf", "v1")]))
        .unwrap();
    f.apply_manifest(&manifest(vec![file("/etc/app.conf", "v2")]))
        .unwrap();
    ...
    let for_file: Vec<_> = steps
        .iter()
        .filter(|s| s.glyph_key.starts_with("file:"))
        .collect();
    assert_eq!(
        for_file.len(),
        2,
        "a file Replace is one apply step: intended + done, no separate reverse"
    );
    assert!(for_file.iter().all(|s| s.action == WalAction::Apply));
    let done = for_file
        .iter()
        .find(|s| s.state == WalStepState::Done)
        .unwrap();
    assert_eq!(
        done.inverse,
...
fn the_applied_fold_matches_the_sequence_of_applies_and_reverses() {
    ...
    f.apply_manifest(&manifest(vec![file("/a", "1"), file("/b", "1")]))
        .unwrap();
    ...
    assert_eq!(
        ...
        "b was removed, a survives at v2"
    );

    let a = &fold[0];
    assert!(
        a.op.glyph_content_id_matches("2"),
        "the survivor is at v2, the applied version"
    );
}
