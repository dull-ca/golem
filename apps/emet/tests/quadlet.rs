...
fn host_mount_source_becomes_a_directory_glyph() {
    ...
    assert!(matches!(entry, Entry::Directory { .. }), "host mount source is a directory");
}
...
fn container_unit_file_renders_every_typed_key() {
    ...
    assert!(unit.contains("Image=docker.io/library/registry:2"), "got:\n{unit}");
    assert!(unit.contains("ContainerName=registry"), "got:\n{unit}");
    assert!(unit.contains("PublishPort=5000:5000/tcp"), "got:\n{unit}");
    assert!(unit.contains("Volume=golem-registry-data.volume:/var/lib/registry:Z"), "got:\n{unit}");
    assert!(unit.contains("Volume=/srv/registry/uploads:/uploads:ro"), "got:\n{unit}");
    assert!(unit.contains("Environment=REGISTRY_LOG_LEVEL=info"), "got:\n{unit}");
    assert!(unit.contains("Restart=always"), "got:\n{unit}");
    assert!(unit.contains("WantedBy=multi-user.target default.target"), "got:\n{unit}");
}
...
fn volume_unit_file_renders_the_dot_volume_quadlet() {
    let c = compiled();
    let vol =
        file_contents(scroll(&c, "registry"), "/etc/containers/systemd/golem-registry-data.volume");
...
fn each_published_port_gets_its_own_firewall_rule() {
    ...
    assert!(nft.contains("5000"), "opens the published port; got:\n{nft}");
    assert!(nft.contains("10.0.0.0/8"), "internal exposure allows the internal network; got:\n{nft}");
...
fn public_exposure_opens_each_port_to_the_world_plus_a_shared_chain_line() {
    ...
    let s = scroll(&c, "web");
    let k = keys(s);
    assert!(k.contains(&"file:/etc/nftables.d/web-443.nft".to_string()), "got: {k:?}");
    assert!(k.contains(&"file:/etc/nftables.d/web-80.nft".to_string()), "got: {k:?}");
    assert!(
        k.iter().any(|key| key.starts_with("fileline:/etc/nftables.d/ingress.nft")),
