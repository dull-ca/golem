from __future__ import annotations

from typing import NamedTuple, Sequence

from excalidraw.layout import connector
from excalidraw.palette import (
    ANSIBLE,
    GOLEM,
    INK_FAINT,
    INK_GHOST,
    INK_SOFT,
    ORANGE,
    WHITE,
    Tone,
)
from excalidraw.scene import CONTENT_WIDTH, MARGIN, Scene
from excalidraw.text import MONO
from excalidraw.type_scale import BODY_SIZE, CAPTION_SIZE

from ..lichess_fleet import HOSTS
from ..machines import Machine, cell_area, draw_machine, swatch_entry
from .glyph_ops import NOOP, Op, op_legend

COLUMN_COUNT = 3
COLUMN_GAP = 46.0
COLUMN_WIDTH = (CONTENT_WIDTH - (COLUMN_COUNT - 1) * COLUMN_GAP) / COLUMN_COUNT

SHOWN_HOSTS: tuple[str, ...] = ("cobar", "dingo", "achoo")

BAND_Y = 186.0
HEADER_CLEARANCE = 12.0
BAND_PADDING = 18.0
BAND_HEADING_WIDTH = 200.0
BAND_FIRST_ROW = 52.0
BAND_ROW_PITCH = 36.0
BAND_MARK = 28.0
BAND_TEXT_GAP = 12.0

MACHINE_Y = 402.0
MACHINE_HEIGHT = 200.0
MACHINE_BOTTOM = MACHINE_Y + MACHINE_HEIGHT

CELL_COLUMNS = 4
CELL_ROWS = 2
CELL_GAP = 4.0
CELL_MARK = 40.0

JOURNAL_Y = 644.0
JOURNAL_PADDING = 18.0
JOURNAL_FIRST_ROW = 46.0
JOURNAL_ROW_PITCH = 30.0
JOURNAL_TONE = Tone(ORANGE, WHITE)

LEGEND_Y = 812.0
NOTE_Y = 858.0

PRESENT_TONE = GOLEM
ABSENT_TONE = Tone(INK_GHOST, WHITE)

PRESENT_CAPTION = "golem keeps this"
ABSENT_CAPTION = "not on the host"


class Cell(NamedTuple):
    present: bool
    op: Op


class PlanRow(NamedTuple):
    op: Op
    count: int
    slots: tuple[int, ...] = ()


class HostPanel(NamedTuple):
    name: str
    cells: tuple[Cell, ...]
    rows: tuple[PlanRow, ...]
    revisions: int


def units_on(name: str) -> int:
    return next(host.tool_units for host in HOSTS if host.name == name)


def column_x(index: int) -> float:
    return MARGIN + index * (COLUMN_WIDTH + COLUMN_GAP)


def cell_rect(x: float, slot: int) -> tuple[float, float, float, float]:
    left, top, width, height = cell_area(
        x, MACHINE_Y, COLUMN_WIDTH, MACHINE_HEIGHT, BODY_SIZE
    )
    cell_width = (width - (CELL_COLUMNS - 1) * CELL_GAP) / CELL_COLUMNS
    cell_height = (height - (CELL_ROWS - 1) * CELL_GAP) / CELL_ROWS
    return (
        left + (slot % CELL_COLUMNS) * (cell_width + CELL_GAP),
        top + (slot // CELL_COLUMNS) * (cell_height + CELL_GAP),
        cell_width,
        cell_height,
    )


def revision_rows(count: int) -> tuple[str, ...]:
    return tuple(
        f"{number}  {'Init' if number == 1 else 'Reconcile'}"
        for number in range(1, count + 1)
    )


def band_height(rows: int) -> float:
    return BAND_FIRST_ROW + rows * BAND_ROW_PITCH + 12.0


def journal_height(revisions: int) -> float:
    return JOURNAL_FIRST_ROW + revisions * JOURNAL_ROW_PITCH + 12.0


def all_cells(name: str, op: Op, present: bool) -> tuple[Cell, ...]:
    return tuple(Cell(present, op) for _ in range(units_on(name)))


def every_slot(name: str) -> tuple[int, ...]:
    return tuple(range(units_on(name)))


def _band(scene: Scene, x: float, panel: HostPanel, heading: str) -> None:
    scene.rectangle(
        x,
        BAND_Y,
        COLUMN_WIDTH,
        band_height(len(panel.rows)),
        Tone(INK_FAINT, WHITE),
        stroke_style="dashed",
    )
    scene.text(
        x + BAND_PADDING,
        BAND_Y + 14,
        panel.name,
        font_size=CAPTION_SIZE,
        colour=INK_SOFT,
        font_family=MONO,
        width=BAND_HEADING_WIDTH,
    )
    scene.text(
        x + COLUMN_WIDTH - BAND_PADDING - BAND_HEADING_WIDTH,
        BAND_Y + 14,
        heading,
        font_size=CAPTION_SIZE,
        colour=INK_FAINT,
        align="right",
        width=BAND_HEADING_WIDTH,
    )
    for position, row in enumerate(panel.rows):
        top = BAND_Y + BAND_FIRST_ROW + position * BAND_ROW_PITCH
        row.op.mark(scene, x + BAND_PADDING, top, BAND_MARK, row.op.tone)
        scene.text(
            x + BAND_PADDING + BAND_MARK + BAND_TEXT_GAP,
            top + (BAND_MARK - CAPTION_SIZE * 1.25) / 2.0,
            row.op.units(row.count),
            font_size=CAPTION_SIZE,
            colour=row.op.tone.stroke,
            font_family=MONO,
            width=COLUMN_WIDTH - 2 * BAND_PADDING - BAND_MARK - BAND_TEXT_GAP,
        )


def _machine(scene: Scene, x: float, panel: HostPanel) -> None:
    draw_machine(
        scene,
        x,
        MACHINE_Y,
        Machine(panel.name, keeper=ANSIBLE, unit_tone=GOLEM, agent=True),
        width=COLUMN_WIDTH,
        height=MACHINE_HEIGHT,
        name_font_size=BODY_SIZE,
    )
    for slot, cell in enumerate(panel.cells):
        left, top, width, height = cell_rect(x, slot)
        scene.rectangle(
            left,
            top,
            width,
            height,
            PRESENT_TONE if cell.present else ABSENT_TONE,
            radius=False,
            stroke_width=2,
            stroke_style="solid" if cell.present else "dotted",
        )
        cell.op.mark(
            scene,
            left + (width - CELL_MARK) / 2.0,
            top + (height - CELL_MARK) / 2.0,
            CELL_MARK,
            cell.op.tone,
        )


def _journal(scene: Scene, x: float, panel: HostPanel) -> None:
    scene.rectangle(
        x,
        JOURNAL_Y,
        COLUMN_WIDTH,
        journal_height(panel.revisions),
        JOURNAL_TONE,
    )
    scene.text(
        x + JOURNAL_PADDING,
        JOURNAL_Y + 14,
        "journal",
        font_size=CAPTION_SIZE,
        colour=ORANGE,
        width=BAND_HEADING_WIDTH,
    )
    for position, row in enumerate(revision_rows(panel.revisions)):
        scene.text(
            x + JOURNAL_PADDING,
            JOURNAL_Y + JOURNAL_FIRST_ROW + position * JOURNAL_ROW_PITCH,
            row,
            font_size=CAPTION_SIZE,
            colour=INK_SOFT,
            font_family=MONO,
            width=COLUMN_WIDTH - 2 * JOURNAL_PADDING,
        )


def _arrows(scene: Scene, x: float, panel: HostPanel) -> None:
    for row in panel.rows:
        if row.op is NOOP or not row.slots:
            continue
        rects = [cell_rect(x, slot) for slot in row.slots]
        left = min(rect[0] for rect in rects)
        right = max(rect[0] + rect[2] for rect in rects)
        top = min(rect[1] for rect in rects)
        middle = (left + right) / 2.0
        connector(
            scene,
            [
                (middle, BAND_Y + band_height(len(panel.rows)) + 8),
                (middle, top - 10),
            ],
            stroke=row.op.tone.stroke,
            stroke_width=3,
        )


def legend(scene: Scene) -> None:
    cursor = op_legend(scene, MARGIN, LEGEND_Y)
    cursor = swatch_entry(
        scene, cursor, LEGEND_Y, PRESENT_TONE, "solid", PRESENT_CAPTION
    )
    swatch_entry(scene, cursor, LEGEND_Y, ABSENT_TONE, "dotted", ABSENT_CAPTION)


def draw(
    scene: Scene,
    panels: Sequence[HostPanel],
    heading: str,
    header_bottom: float,
) -> None:
    if header_bottom > BAND_Y - HEADER_CLEARANCE:
        raise ValueError(
            f"the header runs to y={header_bottom:.0f} and the plan band starts "
            f"at y={BAND_Y:.0f}: shorten the title or the subtitle"
        )
    for index, panel in enumerate(panels):
        x = column_x(index)
        _band(scene, x, panel, heading)
        _machine(scene, x, panel)
        _journal(scene, x, panel)
        _arrows(scene, x, panel)
    legend(scene)
