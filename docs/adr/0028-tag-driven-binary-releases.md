...
Accepted — Dr. Dub ratified the default. **Not yet activated.**

The decision below is the ratified release policy. The first real publish is
gated on three **activation prerequisites** (see the checklist at the end of the
Decision), none of which is met on the branch that carries this ADR:

- the repository is pushed to the git host (the release branch is currently
  local/unpushed);
...
the repo does not yet live on. This ADR is therefore **Accepted policy, pending
activation** — ready for a CI-scaffold implementation pass.
